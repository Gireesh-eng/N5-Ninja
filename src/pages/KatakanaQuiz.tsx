...
import { CheckCircle, XCircle, Award, Sparkles, ArrowRight, Loader2, Play } from 'lucide-react';
import confetti from 'canvas-confetti';
import { scriptDataService, QuizQuestion, CharacterCategory, IScriptDataService } from '@/lib/ScriptDataService';

// Extend the service interface to include Katakana-specific methods
interface ExtendedScriptDataService extends IScriptDataService {
  getKatakanaCategories: () => Promise<CharacterCategory[]>;
  getKatakanaMainRows: () => Promise<string[]>;
  getKatakanaDakutenRows: () => Promise<string[]>;
  getKatakanaQuizQuestions: (category: CharacterCategory, selectedRows?: string[]) => Promise<QuizQuestion[]>;
}
...
const KatakanaQuiz: React.FC = () => {
  ...
  const [mainRows, setMainRows] = useState<string[]>([]); // State for main rows
  const [dakutenRows, setDakutenRows] = useState<string[]>([]); // State for dakuten rows
    ...
    async function loadInitialData() {
      ...
      try {
        ...
        const [cats, main, dakuten] = await Promise.all([
          service.getKatakanaCategories(),
          service.getKatakanaMainRows(), // Fetch main rows
          service.getKatakanaDakutenRows() // Fetch dakuten rows
        ]);
        ...
        console.error("Failed to load katakana data:", err);
  ...
  const startQuiz = async () => {
    ...
    try {
      const service = scriptDataService as unknown as ExtendedScriptDataService;
      const questions = await service.getKatakanaQuizQuestions(selectedCategory, selectedRows);

      if (questions.length === 0) {
        setError(`No katakana characters found for the selected criteria. Please adjust your selection.`);
        setIsLoading(false);
        return;
      }
      ...
      console.error("Failed to load katakana quiz data:", err);
  ...
  const getScoreFeedback = () => {
    ...
    if (percentage >= 90) return "Excellent! You're a katakana master!";
    if (percentage >= 70) return "Great job! You're getting good at katakana!";
    if (percentage >= 50) return "Good effort! Keep practicing your katakana.";
    return "You're on your way to learning katakana. Keep practicing!";
  };

  // Category name formatter
  const formatCategoryName = (category: string) => {
    if (category === 'all') return 'All Katakana';
    ...
    <Button
      ...
      className={`
        ...
        ${selectedCategory === category 
          ...
          : "bg-white hover:bg-purple-50 text-purple-600 border-purple-200 hover:border-purple-300 dark:bg-gray-900 dark:text-gray-100 dark:border-gray-700 dark:hover:bg-gray-800"
        }
      `}
    ...
    return (
      <Layout>
        <div className="container mx-auto px-4 py-8 min-h-screen flex flex-col items-center justify-center">
          ...
          <p className="text-lg text-gray-700 dark:text-gray-300">Loading Katakana Quiz...</p>
        </div>
      </Layout>
    );
  }
    ...
    return (
      <Layout>
        <div className="container mx-auto px-4 py-8">
          ...
          >
            <Card className="border-purple-100 dark:border-purple-900 shadow-lg overflow-hidden">
              <div className="bg-gradient-to-r from-purple-500 to-violet-600 p-6">
                <h1 className="text-3xl font-bold text-white">Katakana Quiz</h1>
                <p className="text-purple-100 mt-2">Test your katakana knowledge!</p>
              </div>
          ...
          {!isQuizComplete ? (
            ...
            >
              <Card className="shadow-lg border-purple-100 dark:border-purple-900 overflow-hidden">
                <CardHeader className="text-center bg-gradient-to-r from-purple-500 to-violet-600 text-white">
                  <Badge variant="outline" className="mb-2 mx-auto py-1 px-4 border-purple-200 text-purple-50 dark:border-purple-600 dark:text-purple-200">
                    {selectedCategory === 'all' ? formatCategoryName(selectedCategory) : `${selectedRows.length} Row(s) Selected`}
                  </Badge>
                  <CardTitle className="text-2xl mb-2 text-white">Katakana Quiz</CardTitle>
                ...
                <CardContent className="pt-6">
                  <div className="flex justify-center mb-8">
                    <motion.div 
                      className="text-7xl font-bold h-32 w-32 flex items-center justify-center bg-purple-50 dark:bg-purple-900/20 rounded-xl shadow-inner"
                            ...
                            className={`flex items-center border rounded-lg p-4 cursor-pointer transition-all ${
                              selectedAnswer === option
                                ? isChecked
                                  ...
                                  : "bg-purple-50 border-purple-400 dark:bg-purple-900/20 dark:border-purple-400"
            ...
            >
              <Card className="shadow-lg border-purple-100 dark:border-purple-900 overflow-hidden">
                <div className="bg-gradient-to-r from-purple-500 to-violet-600 p-6 text-center">
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    transition={{ 
                  ...
                  >
                    ...
                    <div className="w-full max-w-md mx-auto mt-4">
                      <div className="h-4 bg-gray-100 dark:bg-gray-800 rounded-full">
                        <motion.div 
                          className="h-4 rounded-full bg-gradient-to-r from-purple-500 to-violet-500" 
                          style={{ width: '0%' }}
